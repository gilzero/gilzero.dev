 ...
 *          "not_show" = true
 *       },
 *      "number_paragraphs_by_revisions" = {
 *          "label" = "Paragraphs with the highest number of revisions",
 *          "description" = "Identify the paragraphs with the highest number of entries in the paragraphs_item_revision table, including revisions across all languages, representing those with the most revisions. The results should display the top twenty nodes with the greatest number of revisions."
 *       }
 *    },
...
final class XrayAuditQueryTaskParagraphsPlugin extends XrayAuditQueryTaskPluginBase {
  ...
  public function getDataOperationResult(string $operation = ''): array {
    ...
    switch ($operation) {
      ...
      case 'number_paragraphs_by_revisions':
        $data = $this->getDataOperationResultCached($cid, 'getParagraphsGroupedByRevision', $cache_tags);
        break;
    }
  ...
  /**
   * Get paragraphs with too many revisions.
   *
   * @return array
   *   Render array.
   */
  public function getParagraphsGroupedByRevision() {
    $alias_count = 'count';
    ...
    $query->addField('pi', 'type', 'paragraph_type');
